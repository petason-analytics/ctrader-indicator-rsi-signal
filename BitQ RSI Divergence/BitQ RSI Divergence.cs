...
namespace cAlgo
{
    ...
    public class BitQRSIDivergence : Indicator
    {
        ...
        public void filterBearish()
        {
            if (peakList.Count >= 2)
            {
                ...
                for (int i = 0; i < peakList.Count; i++)
                {
                    ...
                    {
                        if(!isBrokenLine(peakList, i, peakList.Count - 1))
                        {
                            if(isDivergencePeak(iPoint, currPoint))
                            {
                                ...
                                IndicatorArea.DrawTrendLine("Bearish_unconfirm1" + iPoint.index + "-" + currPoint.index, iPoint.index, iPoint.value, currPoint.index, currPoint.value, Color.Green, 3);
                            }
                        }
                    }
                }
        ...
        public bool isBrokenLine(ArrayList array, int startIndex, int endIndex)
        {
            bool isBrokenLine = false;
            if (endIndex - startIndex == 1) return false;
            double b, w;
            findLine((Point)peakList[startIndex], (Point)peakList[endIndex], out b, out w);
